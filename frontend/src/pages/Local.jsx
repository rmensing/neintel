...
function openNav(id) {
    console.log("GET",document.getElementById("mySidenav"+id),"mySidenav"+id);
...
function closeNav(id) {
    console.log("GET",document.getElementById("mySidenav"+id),"mySidenav"+id);
    if (document.getElementById("mySidenav"+id) != null){
    document.getElementById("mySidenav"+id).style.width = "0px";
                                ...
                                <Match when={char.ships_all.length > 0}>
                                    ...
                                    {(shp,s) => (
                                        ...
                                        <div class="grid grid-cols-3 gap-0 bg-gray-800 border-b border-gray-700 text-gray-100">
                                            ...
                                            <div><img data-popover-target={"all-"+ char.id +"-"+ shp.shipTypeID} data-popover-placement="left" src={"https://images.evetech.net/types/"+ shp.shipTypeID +"/icon"} title={""+ ships.get(shp.shipTypeID).typeName +""} class="rounded w-8 h-8 border-0"></img></div>
                                            <div>{ships.get(shp.shipTypeID).typeName}</div>
                                        </div>
                                        <Portal>
                                            <div data-popover id={"all-"+ char.id +"-"+ shp.shipTypeID} role="tooltip" class="absolute z-50 invisible inline-block w-96 text-sm text-gray-500 transition-opacity duration-300 bg-white border border-gray-200 rounded-lg shadow-sm opacity-0 dark:text-gray-400 dark:border-gray-600 dark:bg-gray-800">
                                                <div class="px-3 py-2 bg-gray-500 border-b border-gray-200 rounded-t-lg dark:border-gray-600 dark:bg-gray-700">
                                                        <h3 class="font-semibold text-xs text-gray-200 dark:text-white">{ships.get(shp.shipTypeID).typeName}</h3>
                                                </div>
